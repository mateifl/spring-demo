		...
		return getRepository()
				...
				.log().map(savedOrder -> {
					...
					Flux.fromIterable(request.getProducts()).flatMap(product -> {
						Mono<OrderDetail> orderDetailMono = orderDetailRepository.save(
								OrderDetail.builder().productId(product.getProductId()).quantity(product.getQuantity())
										.orderId(savedOrder.getId()).unitPrice(product.getUnitPrice()).build());
						if(log.isDebugEnabled()) log.debug("order detail saved");
