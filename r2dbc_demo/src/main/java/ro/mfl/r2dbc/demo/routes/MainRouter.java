...
public class MainRouter {
  ...
  RouterFunction<ServerResponse> productsRoutes(ProductHandler productHandler) {
    return route(GET("/products"), productHandler::getEntities).and(route(GET("/products/{id}"), productHandler::getEntity));
  }
  
  @Bean
  RouterFunction<ServerResponse> categoriesRoutes(CategoryHandler categoryHandler) {
    return route(GET("/categories"), categoryHandler::getEntities).and(route(GET("/categories/{id}"), categoryHandler::getEntity));
  }
